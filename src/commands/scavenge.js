  ...
  async execute(interaction) {
    ...
    sqlite.transaction(() => {
      ...
      if (loot.type === 'yen') {
        ...
        db.update(players).set({ yen: fresh.yen + amount, last_hunt_at: now }).where(eq(players.discord_id, userId)).run();
        result = { desc: `**${amount} 💰**`, label: loot.label };
      } else {
        if (!freshJob.__items) freshJob.__items = [];
        freshJob.__items.push(loot.itemKey);
        db.update(players).set({ job_data: JSON.stringify(freshJob), last_hunt_at: now }).where(eq(players.discord_id, userId)).run();
