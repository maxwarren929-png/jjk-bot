...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'list') {
      const bountiesList = listBounties();
      if (bountiesList.length === 0) return interaction.editReply('❌ No active bounties.');

      const sorted = bountiesList.sort((a, b) => b.total - a.total);
      ...
      const embed = new EmbedBuilder()
        .setTitle(`💰 Active Bounties (${bountiesList.length} targets)`)
        .setColor(0xF1C40F)
